    ...
    if not st.session_state.orders:
        ...
        if st.button("📥 御見積書 PDF生成"):
            try:
                ...
                # フォント設定（日本語対応：環境に合わせて変更、クラウドの場合は同梱フォント推奨）
                font_path = "C:/Windows/Fonts/meiryo.ttc"
                if os.path.exists(font_path):
                    pdfmetrics.registerFont(TTFont('Meiryo', font_path))
                    base_font = 'Meiryo'
                else:
                    base_font = 'Helvetica' # フォールバック
                ...
                total_amount = 0
                shipping = client_orders[0]["get", "送料", 1000] if "送料" in client_orders[0] else 1000
                tax_rate = client_orders[0].get("税率", 0.10)
