...
def search_unit_price(
  ...
  if matched_col_index is None:
    raise ValueError(
        f"品番 '{target_item}' とカラー '{target_color}'"
        " の価格列が見つかりませんでした。"
    )
...
if app_mode == "👕 1. ボディ発注（カート・注文書作成）":
  ...
  st.write(
      "メーカーの価格リストからボディを選び、一度カートに入れてから注文を確定・発注書を発行します。"
  )
  ...
  if st.button("🛒 カートに追加する"):
    ...
    for item in st.session_state.cart_items:
      if (
          item["maker"] == maker_input
          and item["item_code"] == selected_item
          and item["color"] == cleaned_color
          and item["size"] == selected_size
      ):
  ...
  if st.session_state.cart_items:
    ...
    for idx, item in enumerate(st.session_state.cart_items):
      ...
      st.markdown(
          f"**{idx + 1}. [{item['maker'].upper()}] {item['item_code']}"
          f" ({item['product_name']})**<br/>"
          f"カラー: {item['color']} / サイズ: {item['size']} / 単価: **{unit_p:,}円**"
          f" × 枚数: **{item['purchase_quantity']}枚** = 小計:"
          f" **{sub_total:,}円**",
    ...
    if st.button("✅ 【注文確定】して在庫リストに追加 ＆ 注文書を有効化する"):
      for cart_item in st.session_state.cart_items:
        ...
        for stock_item in st.session_state.confirmed_stock:
          if (
              stock_item["maker"] == cart_item["maker"]
              and stock_item["item_code"] == cart_item["item_code"]
              and stock_item["color"] == cart_item["color"]
              and stock_item["size"] == cart_item["size"]
          ):
      ...
      st.session_state["last_ordered_items"] = (
          st.session_state.cart_items.copy()
      )
  ...
  if st.session_state.order_completed and st.session_state["last_ordered_items"]:
    ...
    with col_dl2:
      try:
        ...
        right_meta_s = ParagraphStyle(
            "OrdRMeta", fontName=FONT_NAME, fontSize=9, alignment=2
        )
        ...
        cell_c = ParagraphStyle(
            "OrdCellC", fontName=FONT_NAME, fontSize=8, alignment=1
        )
        cell_r = ParagraphStyle(
            "OrdCellR", fontName=FONT_NAME, fontSize=8, alignment=2
        )
        ...
        sum_val_style = ParagraphStyle(
            "SumVal", fontName=FONT_NAME, fontSize=8, textColor=colors.black, alignment=2
        )

        elements = []

        header_banner_data = [[
            Paragraph("<b>ボ デ ィ 発 注 書</b>", t_style),
            Paragraph(
        ...
        elements.append(
            Paragraph(
                "メーカー各社ご担当者様<br/>下記のとおり発注いたします。", meta_s
            )
        )
        elements.append(Spacer(1, 10))

        table_data = [[
            Paragraph("メーカー", hdr_s),
            Paragraph("品番 ／ 商品名称", hdr_s),
            Paragraph("カラー / サイズ", hdr_s),
            Paragraph("単価", hdr_s),
            Paragraph("数量", hdr_s),
            Paragraph("小計", hdr_s),
        ]]
        ...
        for item in st.session_state["last_ordered_items"]:
          ...
          table_data.append([
              ...
              Paragraph(
                  f"<b>{item['item_code']}</b><br/>{item.get('product_name', '')}",
                  cell_s,
              ),
        ...
        for _ in range(
            max(0, 10 - len(st.session_state["last_ordered_items"]))
        ):
          table_data.append([
              Paragraph("", cell_s),
              Paragraph("", cell_s),
        ...
        for r_idx in range(1, len(table_data)):
          if r_idx % 2 == 1:
            ts.append(
                ("BACKGROUND", (0, r_idx), (-1, r_idx), colors.HexColor("#F2F4F7"))
            )
          else:
        ...
        sum_table_data = [[
            Paragraph("総発注枚数", sum_hdr_style),
            Paragraph(f"{total_order_qty:,} 枚", sum_val_style),
            Paragraph("合計金額 (税抜)", sum_hdr_style),
            Paragraph(f"¥ {total_order_price:,}", sum_val_style),
        ]]
...
# ==========================================
  ...
  if not st.session_state.confirmed_stock:
    ...
    for idx, stock_item in enumerate(st.session_state.confirmed_stock):
      with st.expander(
          f"📦 [{stock_item['maker'].upper()}] 品番:"
          f" {stock_item['item_code']} - {stock_item['color']} /"
          f" {stock_item['size']} (現在庫:"
          f" {stock_item['purchase_quantity']}枚)"
      ):
        ...
        with col_s1:
          new_qty = st.number_input(
              "在庫数変更",
              min_value=0,
              value=int(stock_item["purchase_quantity"]),
              key=f"stock_edit_qty_{idx}",
          )
        with col_s2:
...
# ==========================================
  ...
  st.write(
      "お見積書を作成・保存すると、下部の履歴一覧からいつでも画像イメージ通りの**「注文請書」**を発行できるようになります。"
  )

  col_input_n1, col_input_n2 = st.columns(2)
  with col_input_n1:
    project_name = st.text_input(
        "案件名", value="〇〇イベント用プリント制作"
    )
  with col_input_n2:
    customer_name = st.text_input(
        "顧客名（お取引先名）", value="株式会社〇〇 御中"
    )

  print_method = st.selectbox(
      "共通プリント方式", ["シルクスクリーン", "インクジェット", "DTF"]
  )
    ...
    for d_i in range(3):
      ...
      with d_cols[0]:
        d_title = st.text_input(
            f"件名 #{d_i+1}",
            value="前面" if d_i == 0 else "",
            key=f"dtf_title_{d_i}",
      ...
      with d_cols[1]:
        d_qty = st.number_input(
            f"面付 #{d_i+1}",
            min_value=0,
            value=1 if d_i == 0 else 0,
            key=f"dtf_qty_{d_i}",
            label_visibility="collapsed",
        )
      with d_cols[2]:
        d_split = st.number_input(
            f"分割 #{d_i+1}",
            min_value=1,
            value=1,
            key=f"dtf_split_{d_i}",
            label_visibility="collapsed",
        )
      with d_cols[3]:
        d_mult = st.number_input(
            f"使用数 #{d_i+1}",
            min_value=1,
            value=1,
            key=f"dtf_mult_{d_i}",
      ...
      if d_qty > 0 and d_title.strip() != "":
        ...
        dtf_summary_parts.append(
            f"{d_title}(面付:{d_qty}/分:{d_split}→{l_cost:,}円)"
        )
  ...
  stock_options = (
      [
          f"[{item['maker'].upper()}] {item['item_code']}"
          f" ({item['product_name']}) - {item['color']} / {item['size']}"
          f" (在庫: {item['purchase_quantity']}枚)"
  ...
  for i in range(15):
    with st.expander(f"➕ 明細行 {i + 1}", expanded=(i == 0)):
      ...
      if line_mode == "📦 確定在庫から選択" and stock_options:
        ...
        try:
          body_cost = search_unit_price(
              EXCEL_FILE,
              f"{chosen['maker']}価格リスト",
              chosen["item_code"],
              chosen["color"],
              chosen["size"],
          )
        ...
        detail_note = st.text_input(
            f"仕様メモ #{i+1}",
            value="カラー: ブラック / サイズ: L" if i == 0 else "",
            key=f"free_note_{i}",
        )
        body_cost = 500
      ...
      with col_qty:
        ...
        qty = st.number_input(
            f"数量 #{i+1}",
            min_value=0,
            max_value=max_qty_limit,
            value=default_qty_val,
            key=f"qty_{i}",
        )
      with col_price:
  ...
  print_process_cost = st.number_input(
      "プリント加工費 総額 (税抜)",
      min_value=0,
      value=dtf_total_cost_calc
      if print_method == "DTF"
      else total_qty_sum * 300,
      step=100,
  )
  ...
  if st.button("🚀 履歴に保存して見積書を発行する", disabled=not est_line_items):
    history_df_new = pd.DataFrame([{
        ...
        "プリント方式": print_method,
        "販売単価": (
            f"{est_line_items[0]['unit_price']:,}円"
            if est_line_items
            else "0円"
        ),
        "数量": total_qty_sum,
    ...
    if os.path.exists(HISTORY_FILE):
      ...
      history_df_combined = pd.concat(
          [history_df_new, history_df_old], ignore_index=True
      )
    else:
      history_df_combined = history_df_new
    history_df_combined.to_csv(HISTORY_FILE, index=False, encoding="utf-8-sig")
    st.success(
        "見積履歴を保存しました！ 下記から見積書PDFをダウンロードできます。"
    )
  ...
  if est_line_items:
    try:
      ...
      right_meta_s = ParagraphStyle(
          "EstRMeta", fontName=FONT_NAME, fontSize=9, alignment=2
      )
      cust_large_s = ParagraphStyle(
          "EstCustLarge", fontName=FONT_NAME, fontSize=13, leading=16
      )

      cell_s = ParagraphStyle("EstCell", fontName=FONT_NAME, fontSize=8)
      cell_c = ParagraphStyle(
          "EstCellC", fontName=FONT_NAME, fontSize=8, alignment=1
      )
      cell_r = ParagraphStyle(
          "EstCellR", fontName=FONT_NAME, fontSize=8, alignment=2
      )
      hdr_s = ParagraphStyle(
          "EstHdr",
          fontName=FONT_NAME,
      ...
      amt_val_s = ParagraphStyle(
          "EstAmtV", fontName=FONT_NAME, fontSize=14, textColor=colors.black, alignment=2
      )
      ...
      header_banner_data = [[
          Paragraph("<b>御 見 積 書</b>", t_style),
          Paragraph(
              f"No : EST-{file_timestamp}<br/>発行日 :"
      ...
      top_info_data = [[Paragraph(cust_text, cust_large_s), right_column_content]]
      ...
      amt_box_data = [[
          Paragraph("御見積金額", amt_title_s),
          Paragraph(f"¥ {total_inc_tax:,} -", amt_val_s),
      ]]
      ...
      for item in est_line_items:
        table_data.append([
            Paragraph(
                f"<b>{project_name}</b><br/>{item['detail_note']}"
                f" ({print_method})",
                cell_s,
            ),
      ...
      sum_table_data = [[
          Paragraph("小計 (税抜)", sum_hdr_style),
          Paragraph(f"¥ {total_sales_amount:,}", sum_val_style),
          Paragraph("消費税 (10%)", sum_hdr_style),
          Paragraph(f"¥ {tax_amount:,}", sum_val_style),
          Paragraph("合計 (税込)", sum_hdr_style),
          Paragraph(f"¥ {total_inc_tax:,}", sum_val_style),
      ]]
      ...
      t_remark = Table(
          remark_data, colWidths=[300], rowHeights=[15, 18, 18, 18]
      )
      t_remark.setStyle(
          TableStyle([
              ("BOX", (0, 0), (-1, -1), 0.5, colors.HexColor("#2E6EA5")),
  ...
  if os.path.exists(HISTORY_FILE):
    ...
    if not history_df.empty:
      ...
      for h_idx, h_row in history_df.iterrows():
        with st.expander(
            f"📁 [{h_row['日時']}] 案件名: {h_row['案件名']} (顧客:"
            f" {h_row['顧客名']} / 金額:"
            f" {int(h_row['売上合計(税抜)']):,}円)"
        ):
          st.markdown("#### ✏️ この履歴の編集・削除・自社/振込先情報の変更")

          edit_col1, edit_col2 = st.columns(2)
          with edit_col1:
            edit_proj_name = st.text_input(
                "案件名編集", value=str(h_row["案件名"]), key=f"edit_proj_{h_idx}"
            )
            edit_cust_name = st.text_input(
                "顧客名編集", value=str(h_row["顧客名"]), key=f"edit_cust_{h_idx}"
            )
            edit_print_m = st.text_input(
                "プリント方式編集",
                value=str(h_row["プリント方式"]),
                key=f"edit_print_{h_idx}",
            )
            ...
            comp_mode = st.radio(
                f"自社情報の入力方法 #{h_idx}",
                ["手入力", "事前登録から選択"],
                key=f"comp_mode_{h_idx}",
                horizontal=True,
            )

            if comp_mode == "事前登録から選択" and not df_cp_load.empty:
              c_options = df_cp_load["名称"].tolist()
              selected_c_preset = st.selectbox(
                  f"登録済み自社情報 #{h_idx}",
                  c_options,
                  key=f"sel_c_pre_{h_idx}",
              )
              matched_cp_row = df_cp_load[
                  df_cp_load["名称"] == selected_c_preset
              ].iloc[0]
              ...
              default_ca = str(
                  h_row.get("自社住所", "〒123-4567 〇〇県〇〇市...")
              )
              default_cs = str(h_row.get("自社担当", "担当 太郎"))

            edit_c_name = st.text_input(
                "自社名", value=default_cn, key=f"edit_c_name_{h_idx}"
            )
            edit_c_addr = st.text_input(
                "自社住所", value=default_ca, key=f"edit_c_addr_{h_idx}"
            )
            edit_c_staff = st.text_input(
                "担当者名", value=default_cs, key=f"edit_c_staff_{h_idx}"
            )
          ...
          with edit_col2:
            edit_qty = st.number_input(
                ...
                min_value=0,
                value=int(h_row["数量"]),
                key=f"edit_qty_{h_idx}",
            )
            edit_sales = st.number_input(
                "売上合計(税抜)編集",
                min_value=0,
                value=int(h_row["売上合計(税抜)"]),
                step=100,
                key=f"edit_sales_{h_idx}",
            )
            ...
            bank_mode = st.radio(
                f"振込先の入力方法 #{h_idx}",
                ["手入力", "事前登録から選択"],
                key=f"bank_mode_{h_idx}",
                horizontal=True,
            )

            if bank_mode == "事前登録から選択" and not df_bp_load.empty:
              b_options = df_bp_load["名称"].tolist()
              selected_b_preset = st.selectbox(
                  f"登録済み振込先 #{h_idx}",
                  b_options,
                  key=f"sel_b_pre_{h_idx}",
              )
              matched_bp_row = df_bp_load[
                  df_bp_load["名称"] == selected_b_preset
              ].iloc[0]
            ...
            edit_bank = st.text_input(
                "銀行名", value=default_bn, key=f"edit_bank_{h_idx}"
            )
            edit_branch = st.text_input(
                "支店名", value=default_bb, key=f"edit_branch_{h_idx}"
            )
            edit_account = st.text_input(
                "口座番号", value=default_ba, key=f"edit_account_{h_idx}"
            )
            edit_holder = st.text_input(
                "口座名義", value=default_bh, key=f"edit_holder_{h_idx}"
            )
          ...
          if st.session_state.get(f"gen_ord_{h_idx}", False):
            ...
            try:
              ...
              cust_large_s = ParagraphStyle(
                  "OrdCustLarge", fontName=FONT_NAME, fontSize=13, leading=16
              )
              ...
              bank_val_s = ParagraphStyle(
                  "BankVal", fontName=FONT_NAME, fontSize=8, alignment=0
              )
              ...
              amt_val_s = ParagraphStyle(
                  "AmtV", fontName=FONT_NAME, fontSize=14, textColor=colors.black, alignment=2
              )
              ...
              header_banner_data = [[
                  Paragraph("<b>注 文 請 書</b>", t_style),
                  Paragraph(
                      f"No : ORD-{h_idx}-{file_timestamp}<br/>発行日 :"
                      f" {datetime.now().strftime('%Y年%m月%d日')}",
                      right_meta_s,
                  ),
              ]]
              ...
              bank_info_data = [
                  ...
                  [
                      Paragraph("支 店", bank_hdr_s),
                      Paragraph(b_branch_val, bank_val_s),
                  ],
                  [
                      Paragraph("口座番号", bank_hdr_s),
                      Paragraph(b_acc_val, bank_val_s),
                  ],
                  [
                      Paragraph("口座名義", bank_hdr_s),
                      Paragraph(b_holder_val, bank_val_s),
                  ],
              ]
              ...
              top_info_data = [[
                  Paragraph(cust_text, cust_large_s),
                  right_column_content,
              ]]
              t_top = Table(top_info_data, colWidths=[310, 235])
              t_top.setStyle(
              ...
              elements_o.append(
                  Paragraph(f"<b>件名：{h_row['案件名']}</b>", meta_s)
              )
              elements_o.append(Spacer(1, 4))
              elements_o.append(
                  Paragraph("下記の通り、ご注文を承りました。", meta_s)
              )
              ...
              notice_p = Paragraph(
                  "<font size=7>※お振込手数料は御社ご負担にてお願いします。</font>",
                  meta_s,
              )

              t_amt_layout = Table([[t_amt, notice_p]], colWidths=[350, 195])
              t_amt_layout.setStyle(
                  TableStyle([("VALIGN", (0, 0), (-1, -1), "BOTTOM")])
              )
              ...
              for r_idx in range(1, len(table_data)):
                if r_idx % 2 == 1:
                  ...
                  ts.append(
                      ("BACKGROUND", (0, r_idx), (-1, r_idx), colors.white)
                  )
              ...
              sum_table_data = [[
                  Paragraph("小計 (税抜)", sum_hdr_style),
                  Paragraph(f"¥ {net_amount:,}", sum_val_style),
                  Paragraph("消費税 (10%)", sum_hdr_style),
                  Paragraph(f"¥ {tax_amount:,}", sum_val_style),
                  Paragraph("合計 (税込)", sum_hdr_style),
                  Paragraph(f"¥ {total_inc_tax:,}", sum_val_style),
...
# ==========================================
  ...
  if not os.path.exists(HISTORY_FILE):
    ...
    total_all_margin = (
        (total_all_profit / total_all_sales * 100) if total_all_sales > 0 else 0
    )
    ...
    with m_col3:
      st.metric(
          "総粗利額 / 平均利益率",
          f"{total_all_profit:,.0f} 円",
          delta=f"{total_all_margin:.1f}%",
      )
    ...
    with col_c1:
      body_total_cost = st.number_input(
          "① ボディ仕入原価 総額 (税抜)",
          min_value=0,
          value=int(target_row["自動ボディ原価"]),
          step=100,
      )
    with col_c2:
      print_process_cost = st.number_input(
    ...
    st.metric(
        "選択案件の粗利額 (利益額)",
        f"{gross_profit:,.0f} 円",
        delta=f"{gross_profit_margin:.1f}% (利益率)",
    )
  ...
  if not os.path.exists(HISTORY_FILE):
    ...
    if st.button("📥 請求書PDFをダウンロード"):
      try:
        ...
        right_meta_s = ParagraphStyle(
            "InvRMeta", fontName=FONT_NAME, fontSize=9, alignment=2
        )
        cust_large_s = ParagraphStyle(
            "InvCustLarge", fontName=FONT_NAME, fontSize=13, leading=16
        )

        cell_s = ParagraphStyle("InvCell", fontName=FONT_NAME, fontSize=8)
        cell_c = ParagraphStyle(
            "InvCellC", fontName=FONT_NAME, fontSize=8, alignment=1
        )
        cell_r = ParagraphStyle(
            "InvCellR", fontName=FONT_NAME, fontSize=8, alignment=2
        )
        hdr_s = ParagraphStyle(
            "InvHdr",
            fontName=FONT_NAME,
        ...
        header_banner_data = [[
            Paragraph("<b>御 請 求 書</b>", t_style),
            Paragraph(
                f"No : INV-{selected_inv_idx}-{datetime.now().strftime('%Y%m%d_%H%M%S')}<br/>発行日 :"
                f" {datetime.now().strftime('%Y年%m月%d日')}",
                right_meta_s,
            ),
        ]]
        ...
        notice_p = Paragraph(
            "<font size=7>※お振込手数料は御社ご負担にてお願いします。</font>", meta_s
        )
        ...
        table_data.append([
            Paragraph(
                f"<b>{inv_project}</b>（{inv_row.get('プリント方式', 'プリント')}）",
                cell_s,
            ),
